...
type ModuleKey = 'ortomosaico' | 'division_potreros' | 'aforo_digital' | 'bromatologia_digital';
type IconClass = `fa-${string}`; 

/**
 * Interface para las propiedades (Props) del componente ServiceCard.
 * (Se mantiene igual)
 */
...
type IconMap = Record<ModuleKey, IconClass>; 


const iconMap: IconMap = {
    ortomosaico: 'fa-map-marked-alt',
    division_potreros: 'fa-fence',
    aforo_digital: 'fa-chart-area',
    bromatologia_digital: 'fa-flask',
};


const ServiceCard: React.FC<ServiceCardProps> = ({ title, description, img, orientation = 'left', module }) => {
    ...
    // **CORRECCIÓN AQUÍ:** Ahora TypeScript sabe que 'module' (tipo ModuleKey)
    // es una clave válida en 'iconMap' (tipo IconMap), ¡eliminando el error 7053 y el any!
    const iconClass = iconMap[module] || 'fa-tools'; // El fallback 'fa-tools' debe ser de tipo IconClass
        ...
        >
            ...
            <div className="md:w-1/2 w-full h-80 md:h-auto relative overflow-hidden">
                <img
                    src={img}
                    alt={title}
                    className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-[1.08] saturate-150 group-hover:saturate-100"
                />
    ...
    return (
        <section 
            className="py-24 px-4 md:px-20 bg-gray-100 
                       ...
                       before:bg-[url('data:image/svg+xml;base64,...')] xl:px-40"
        >
            <div className="max-w-7xl xl:max-w-full mx-auto relative z-10">
